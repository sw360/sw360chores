...
You need to create and place the dependencies for building the docker images at
the correct places. The dependencies are
- `./sw360/sw360_dependencies.tar.gz` containing the modified tomcat, together
    with liferay
- `./couchdb-lucene/couchdb-lucene-1.1.0-dist.zip` containing the compiled couchdb-lucene

For this you can use the `./docker-compose.sh` wrapper by calling
```
