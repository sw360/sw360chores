...
[ -f "$DEPLOYMENT/configuration.env" ] && source "$DEPLOYMENT/configuration.env"
...
assertTomcat () {
    ...
    set -x
    $DEPLOYMENT/docker-compose.sh logs sw360 2>/dev/null | grep -iq "Loading file:/opt/sw360/webapps/ROOT/WEB-INF/classes/portal-ext.properties"
    $DEPLOYMENT/docker-compose.sh logs sw360 2>/dev/null | grep -iq "Determine dialect for PostgreSQL 9"
    $DEPLOYMENT/docker-compose.sh logs sw360 2>/dev/null | grep -iq "org.apache.catalina.startup.Catalina.start Server startup in"
    set +x
}
...
assertCouchdb () {
    ...
    set -x
    $DEPLOYMENT/docker-compose.sh logs sw360couchdb 2>/dev/null | grep -iq "Apache CouchDB has started"
    set +x
}
