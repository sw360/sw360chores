...
LIFERAY_URL="https://downloads.sourceforge.net/project/lportal/Liferay%20Portal/7.3.3%20GA4/"
...
if [ ! -f "$TARGET" ]; then
    mkdir -p downloads
    cd downloads
    echo "... download of MD5 checksum"
    curl -OsLC - "$LIFERAY_URL""$LIFERAY.MD5"
    LIFERAY_EXPECTED_CHECKSUM=$(cat "$LIFERAY.MD5" | cut -f 1 -d ' ')
    echo "... expecting checksum $LIFERAY_EXPECTED_CHECKSUM"

    if [ ! -f "$LIFERAY" ]; then
        echo "... start downloading $LIFERAY (this can take some time)"
        curl -OsLC - "$LIFERAY_URL""$LIFERAY"
    else
    ...
    echo "... got checksum $LIFERAY_ACTUAL_CHECKSUM"
    if [[ "$LIFERAY_ACTUAL_CHECKSUM" != "$LIFERAY_EXPECTED_CHECKSUM" ]]; then
        echo $LIFERAY_ACTUAL_CHECKSUM
        echo "the checksum of $DIR/$LIFERAY.tar.gz does not match"
    ...
    downloadModule "https://search.maven.org/remotecontent?filepath=com/google/code/gson/gson/2.8.6/gson-2.8.6.jar"
    ...
    cd "$TMP"
    mv liferay* sw360
    mv sw360/tomcat* sw360/tomcat
    echo "$LIFERAY" > sw360/liferay.version
